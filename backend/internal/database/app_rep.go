...
func (r *AppRepository) CreateApp(ctx context.Context, app *models.App) error {
	query := `
		INSERT INTO apps (title, description, size_mb, age_rating, downloads, rating, version, developer_id, category_id)
...
func (r *AppRepository) GetApp(ctx context.Context, id int) (*models.App, error) {
	...
	query := `
		SELECT id, title, description, size_mb, age_rating, downloads, rating, version, developer_id, category_id
	...
	err := row.Scan(
		...
		&app.AgeRating, &app.Downloads, &app.Rating, &app.Version,
